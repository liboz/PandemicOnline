...
Player.prototype.dispatcher_move = function(
  game,
  other_player,
  final_destination,
  socket = null
...
Player.prototype.canDirectFlight = function(
  final_destination,
  hand = this.hand
) {
...
Player.prototype.directFlight = function(
  game,
  final_destination,
  hand,
  socket
...
Player.prototype.charterFlight = function(
  game,
  final_destination,
  hand,
  socket
...
Player.prototype.operationsExpertMove = function(
  game,
  final_destination,
  card,
  socket = null
...
Player.prototype.canOperationsExpertMove = function(game) {
  return (
    this.role === other.Roles.OperationsExpert &&
...
Player.prototype.get_valid_final_destinations = function(
  game,
  hand = this.hand
) {
...
Player.prototype.can_build_research_station = function(game) {
  return (
    !game.game_graph[this.location].hasResearchStation &&
    (this.hand.has(this.location) || this.role === other.Roles.OperationsExpert)
  );
};
    ...
    return [...game.game_graph[this.location].players].some(player => {
      if (player !== this) {
        return (
          ...
          (player.role === other.Roles.Researcher && player.hand.size > 0)
        );
      }
    });
  }
...
Player.prototype.can_take_from_player = function(player, card = null) {
  if (this.location !== player.location) {
    ...
    if (card) {
      return (
        player.hand.has(this.location) ||
        (player.role === other.Roles.Researcher && player.hand.has(card))
      );
    } else {
...
Player.prototype.can_give = function(game) {
  if (game.game_graph[this.location].players.size <= 1) {
    ...
    return (
      ...
      (this.role === other.Roles.Researcher && this.hand.size > 0)
    );
  }
};
...
Player.prototype.can_give_card = function(game, card) {
  return (
    this.can_give(game) &&
    this.role === other.Roles.Researcher &&
    this.hand.has(card)
  );
};
